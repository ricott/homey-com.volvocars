...
class ConnectedVehicle {
    ...
    isEngineRunning(vin) {
        return this.#invoke('get', `/connected-vehicle/v2/vehicles/${vin}/engine-status`)
            .then(result => {
                if (result?.data?.engineStatus?.value == 'RUNNING') {
                    return true;
                } else {
                    return false;
                }
        ...
        return this.#invoke('get', `/energy/v1/vehicles/${vin}/recharge-status/battery-charge-level`)
            .then(result => {
                if (result?.data?.batteryChargeLevel?.value &&
                    ...
                    return parseFloat(result?.data?.batteryChargeLevel?.value);
                } else {
                    return null;
                }
            })
    ...
    getChargingSystemStatus(vin) {
        return this.#invoke('get', `/energy/v1/vehicles/${vin}/recharge-status/charging-system-status`)
            .then(result => {
                return result?.data?.chargingSystemStatus?.value;
            })
            .catch(reason => {
                return Promise.reject(reason);
            });
        ...
        try {
            ...
            if (result.response.statusCode > 199 && result.response.statusCode < 300) {
                ...
                try {
                    ...
                    console.error(`Error description: ${json.error?.description || ''}`);
                    console.error(`Error detail: ${json.error?.detail || ''}`);
                } catch (ignore) { }
