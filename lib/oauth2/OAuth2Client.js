...
class OAuth2Client extends EventEmitter {
  ...
  constructor({
    ...
    // Diagnostics for token-refresh reliability over time. These let failure
    // telemetry report how long the token had been working before it broke and
    // how many times in a row it has failed.
    this._lastSuccessfulRefreshAt = null;
  ...
  async _doRefreshToken() {
    ...
    // Track success so a later failure can report how long the token had been
    // working, and reset the consecutive-failure counter.
  ...
  async onHandleGetTokenByCodeResponse({ response }) {
    ...
    // Map token response and add timestamp. This is the initial authorization,
    // so the session and the refresh token both start their clocks now.
    const now = Date.now();
    const tokenData = {
  ...
  async onHandleRefreshTokenResponse({ response }) {
    ...
    const now = Date.now();
    const rotated = !!json.refresh_token && json.refresh_token !== oldToken?.refresh_token;
    const tokenData = {
      access_token: json.access_token,
      refresh_token: json.refresh_token || oldToken?.refresh_token,
      token_type: json.token_type || 'Bearer',
      expires_in: json.expires_in,
      created_at: now,
      // Carry the session origin across rotations and restarts; only reset the
      // refresh-token issue time when the refresh token actually rotates. Fall
      // back to created_at for sessions saved before these fields existed.
      session_created_at: oldToken?.session_created_at || oldToken?.created_at || now,
      refresh_token_created_at: rotated
        ? now
        : (oldToken?.refresh_token_created_at || oldToken?.created_at || now),
    };
    ...
    if (rotated) {
      this.debug('Refresh token rotated - resetting refresh-token age clock');
    }
  ...
  async onHandleRefreshTokenError({ response, error }) {
    ...
    // The OAuth2 error code (e.g. 'invalid_grant') and its human description are
    // the single most useful signal for understanding *why* a refresh fails over
    // time, so capture them verbatim (original case) - separately from the
    // lowercased blob used for classification below.
    let oauthError = null;
    ...
    if (normalizedErrorBody) {
      try {
        ...
        // Standard OAuth2 error shape is { error, error_description }; fall back
        // to nested / alternative fields other providers use.
        const rawError = (typeof parsedBody?.error === 'object')
          ? (parsedBody?.error?.error || parsedBody?.error?.code)
          : parsedBody?.error;
        oauthError = rawError ? String(rawError) : null;

        const rawDescription = parsedBody?.error_description
          || parsedBody?.message
          || parsedBody?.errorMessage
          || ((typeof parsedBody?.error === 'object') ? parsedBody?.error?.message : null);
        oauthErrorDescription = rawDescription ? String(rawDescription) : null;
    ...
    // Gather timing context from the *current* (soon-to-be-replaced) token. This
    // is what lets us reason about failures "over time": how old the access
    // token was, whether it was still considered refreshable, and how long since
    // the last successful refresh.
    const staleToken = this.getToken?.();
    const tokenCreatedAt = staleToken?.created_at || null;
    const accessTokenAgeMs = tokenCreatedAt ? (Date.now() - tokenCreatedAt) : null;
    const msToExpiry = staleToken?.getTimeToExpiry?.() ?? null;
    const refreshable = staleToken?.isRefreshable?.() ?? null;
    const msSinceLastSuccess = this._lastSuccessfulRefreshAt
      ? (Date.now() - this._lastSuccessfulRefreshAt)
      : null;
    // Age of the refresh token itself (the long-lived credential) and of the
    // whole session. These are the key signals for telling a time-based token
    // death apart from a lost-rotation / network-correlated death.
    const refreshTokenAgeMs = staleToken?.refresh_token_created_at
      ? (Date.now() - staleToken.refresh_token_created_at)
      : null;
    const sessionAgeMs = staleToken?.session_created_at
      ? (Date.now() - staleToken.session_created_at)
      : null;
    this._consecutiveRefreshFailures = (this._consecutiveRefreshFailures || 0) + 1;

    // Store error information for retry logic (and so the device layer can read
    // the reason when it handles the resulting 'expired' event).
    ...
    // Emit a semantic telemetry event so the app layer (which knows the session
    // id) can forward it to Sentry. The client itself stays decoupled from any
    // telemetry backend - this mirrors the existing 'log'/'error'/'expired'
    // event pattern. An event with no listener is a harmless no-op.
    this.emit('telemetry', {
      kind: 'refresh_token_error',
      errorType,
      status: status || null,
      oauthError,
      oauthErrorDescription,
      message: this._lastRefreshError.message,
      accessTokenAgeMs,
      refreshTokenAgeMs,
      sessionAgeMs,
      msToExpiry,
      refreshable,
      msSinceLastSuccess,
      consecutiveFailures: this._consecutiveRefreshFailures,
    });
