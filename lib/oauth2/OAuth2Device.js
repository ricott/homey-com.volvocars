 ...
 * @description Base class for OAuth2-enabled devices
 */
class OAuth2Device extends Homey.Device {
  ...
  async onInit() {
    ...
    if (typeof this.onOAuth2Migrate === 'function') {
      try {
        const { OAuth2SessionId, OAuth2ConfigId } = this.getStore();
        ...
        if (!OAuth2SessionId || !OAuth2ConfigId) {
          ...
          const { sessionId, configId, token, title = null } = result;

          let client;
          const hasClient = this.homey.app.hasOAuth2Client({ sessionId, configId });
          if (!hasClient) {
