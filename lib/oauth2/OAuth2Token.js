...
class OAuth2Token {
  ...
  constructor({
    ...
    // Diagnostics (not used for expiry logic): when the overall session was
    // first authorized (survives refresh-token rotation), and when the current
    // refresh token was issued (reset only on rotation). Both persist across
    // restarts via toJSON(). Null for tokens saved before this was introduced.
    this.session_created_at = session_created_at || null;
    this.refresh_token_created_at = refresh_token_created_at || null;
  }
  ...
  toJSON() {
    return {
      ...
      refresh_token_created_at: this.refresh_token_created_at,
    };
  }
