...
/**
 * @extends Homey.App
 * @description Base class for OAuth2-enabled apps
 */
class OAuth2App extends Homey.App {
   ...
   * @param {string} args.configId
   */
  checkHasConfig({ configId = 'default' } = {}) {
   ...
   * @returns {boolean}
   */
  hasOAuth2Client({ sessionId, configId = 'default' } = {}) {
    this.checkHasConfig({ configId });
   ...
   * @param {string} args.configId
   */
  checkHasOAuth2Client({ sessionId, configId = 'default' } = {}) {
  ...
  createOAuth2Client({ sessionId, configId = 'default' } = {}) {
    ...
    // Forward semantic telemetry events (emitted by OAuth2Client) to Sentry.
    // The app layer is where the session id is known; the client stays backend
    // agnostic. Wrapped so a telemetry failure can never break the client.
    clientInstance.on('telemetry', (info = {}) => {
      try {
        if (info.kind === 'refresh_token_error') {
          logger.reportRefreshTokenError({
            errorType: info.errorType,
            status: info.status,
            oauthError: info.oauthError,
            oauthErrorDescription: info.oauthErrorDescription,
            message: info.message,
            accessTokenAgeMs: info.accessTokenAgeMs,
            refreshTokenAgeMs: info.refreshTokenAgeMs,
            sessionAgeMs: info.sessionAgeMs,
            msToExpiry: info.msToExpiry,
            refreshable: info.refreshable,
            msSinceLastSuccess: info.msSinceLastSuccess,
  ...
  deleteOAuth2Client({ sessionId, configId = 'default' } = {}) {
    ...
    if (savedSession && savedSession.configId === configId) {
  ...
  getOAuth2Client({ sessionId, configId = 'default' } = {}) {
    ...
    if (savedSessions && savedSessions[sessionId]) {
      ...
      const { token: Token } = this.getConfig({ configId });

      const client = this.createOAuth2Client({ sessionId, configId });
      ...
      this.tryCleanSession({ sessionId, configId });
  ...
  getFirstSavedOAuth2Client() {
    ...
    return this.getOAuth2Client({ configId, sessionId });
  }
   ...
   * @param {string} args.configId
   */
  tryCleanSession({ sessionId, configId = 'default' }) {
    Promise.resolve().then(async () => {
      const shouldDeleteSession = await this.onShouldDeleteSession({ sessionId, configId });
   ...
   */
  async getOAuth2Devices({ sessionId, configId = 'default' }) {
    let result = [];
    ...
    for (const driverId of this.constructor.OAUTH2_DRIVERS) {
      ...
      const devices = driverInstance.getDevices().filter(device => {
        const { OAuth2ConfigId, OAuth2SessionId } = device.getStore();
