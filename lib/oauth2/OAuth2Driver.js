 ...
 * @description Base class for OAuth2-enabled drivers
 */
class OAuth2Driver extends Homey.Driver {
  ...
  onPair(socket) {
    ...
    const OAuth2Config = this.homey.app.getConfig({ configId: OAuth2ConfigId });
        ...
        oAuth2Callback
          ...
          .on('code', code => {
            client.getTokenByCode({ code })
              ...
              .catch(err => {
                logger.reportAuthFlowError('pair', err);
    ...
    const onListSessions = async () => {
      ...
      result.push({
        name: this.constructor.OAUTH2_NEW_SESSION_TITLE,
        icon: this.constructor.OAUTH2_NEW_SESSION_ICON,
        data: { id: '$new' },
      });
   ...
   * @description Can be extended
   * @param {object} args
   * @param {OAuth2Client} args.oAuth2Client
        ...
        oAuth2Callback
          ...
          .on('code', code => {
            client.getTokenByCode({ code })
              ...
              .catch(err => {
                logger.reportAuthFlowError('repair', err);
                socket.emit('error', err.message || err.toString()).catch(this.error);
              });
          });
      } catch (err) {
