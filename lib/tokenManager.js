    ...
    getToken(vccLoginToken, username, password, force) {
        ...
        return mutex.acquire()
            .then(function (release) {
                ...
                if (self.userTokens[username] == null) {
                    return generateNewToken(vccLoginToken, username, password)
                        .then(function (token) {
                            self.userTokens[username] = new VolvoToken(vccLoginToken, username, password, token);
                            return Promise.resolve(self.userTokens[username].token);
                        }).catch(function (reason) {
                            return Promise.reject(reason);
                        });
                } else if (force) {
                    const now = new Date().getTime();
                    let tokenAge = now - self.userTokens[username].timestamp;
...
class VolvoToken {
    constructor(vccLoginToken, username, password, token) {
        ...
        this._refreshTokenInterval = (token.expires_in || 1500) - 120;
        ...
        }, this._refreshTokenInterval * 1000);
    }
