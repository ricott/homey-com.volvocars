...
class AuthHandler {
    ...
    // Helper method to create timeout signal
    #createTimeoutSignal() {
        return AbortSignal.timeout(config.apiTimeout);
    }
    ...
    async #verifyUsernamePassword(inputOptions) {
        ...
        const url = `${inputOptions.path}?action=checkUsernamePassword`;
    ...
    async #continueAuth(inputOptions) {
        ...
        const url = `${inputOptions.path}?action=continueAuthentication`;

        try {
            const response = await fetch(url, options);

            if (response.ok) {
                console.log(`[${username}] Continue auth successful`);
                const data = await response.json();
                return {
                    cookie: response.headers.get('set-cookie'),
                    data: data
    ...
    async #verifyOtp(inputOptions) {
        ...
        const url = `${inputOptions.path}?action=checkOtp`;

        try {
            const response = await fetch(url, options);

            if (response.ok) {
                console.log(`[${username}] OTP verification successful`);
                const data = await response.json();
                return {
                    cookie: response.headers.get('set-cookie'),
                    data: data
    ...
    async #getToken(inputOptions) {
        ...
        try {
            ...
            if (response.ok) {
                console.log(`[${username}] Token received`);
