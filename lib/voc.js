...
class VOC extends EventEmitter {
    ...
    getVehicleAttributes(vehicleId) {
        let self = this;
    ...
    getVehicleStatusFromCloud(vehicleId) {
        let self = this;
    ...
    getVehicleChargeLocations(vehicleId) {
        let self = this;
    ...
    refreshVehicleStatusFromCar(vehicleId) {
        let self = this;
    ...
    startPreClimatization(vehicleId) {
        let self = this;
    ...
    stopPreClimatization(vehicleId) {
        let self = this;
    ...
    startEngine(vehicleId, duration) {
        let self = this;
    ...
    delayCharging(vehicleId, chargeLocationId, payload) {
        let self = this;
    ...
    honkHornAndBlinkLights(vehicleId, latitude, longitude) {
        let self = this;
    ...
    honkHorn(vehicleId, latitude, longitude) {
        let self = this;
    ...
    blinkLights(vehicleId, latitude, longitude) {
        let self = this;
    ...
    #postWaitForResponse(url, vehicleId, payload) {
        let self = this;
    ...
    #postWithPositionWaitForResponse(url, vehicleId, latitude, longitude) {
        let self = this;
...
async function getVOCCommand(inputOptions, path) {
    const options = {
        ...
        headers: {
            ...
            'Accept': '*/*',
            'Authorization': 'Basic ' + Buffer.from(`${inputOptions.username}:${inputOptions.password}`).toString('base64')
        }
    };

    const url = `https://${apiDomains[inputOptions.region]}${apiEndpoint}${path}`;
    ...
    try {
        ...
        if (error.name === 'TimeoutError') {
            return Promise.reject(new Error(`Request timed out after ${config.apiTimeout}ms`));
        }
        return Promise.reject(error);
    }
}

async function postVOCCommand(inputOptions, path, data) {
    let options = {
        method: 'POST',
        signal: AbortSignal.timeout(config.apiTimeout),
        headers: {
...
async function postVOCCommandwithPosition(inputOptions, path, latitude, longitude) {
    const options = {
        method: 'POST',
        signal: AbortSignal.timeout(config.apiTimeout),
        headers: {
...
async function putVOCCommand(inputOptions, path, data) {
    let options = {
        method: 'PUT',
        signal: AbortSignal.timeout(config.apiTimeout),
        headers: {
