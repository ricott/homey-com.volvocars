...
VOC.prototype.login = function () {
  var self = this;
  return login(self.username, self.password, self.region)
...
VOC.prototype.startPreClimatization = function (vehicleId) {
  var self = this;
  return startPreClimatization(self.username, self.password, self.region, vehicleId)
    .then(function (status) {
      return awaitSuccessfulServiceInvocation(self, vehicleId, status.customerServiceId)
        .then(function (result) {
...
VOC.prototype.stopPreClimatization = function (vehicleId) {
  var self = this;
  return stopPreClimatization(self.username, self.password, self.region, vehicleId)
    .then(function (status) {
      return awaitSuccessfulServiceInvocation(self, vehicleId, status.customerServiceId)
        .then(function (result) {
...
function login(user, pwd, region) {
  if (!user) return Promise.reject(new Error('no_username'));
  if (!pwd) return Promise.reject(new Error('no_password'));
...
function startPreClimatization(user, pwd, region, vehicleId) {
  return postVOCCommand(user, pwd, region, `vehicles/${vehicleId}/preclimatization/start`, null)
    .then(function (data) {
      if (!data) return Promise.reject(new Error('startPreClimatization, api_error'));
      return data;
  });
}
function stopPreClimatization(user, pwd, region, vehicleId) {
  return postVOCCommand(user, pwd, region, `vehicles/${vehicleId}/preclimatization/stop`, null)
    .then(function (data) {
...
function getVehiclePosition(user, pwd, region, vehicleId) {
  return getVOCCommand(user, pwd, region, `vehicles/${vehicleId}/position?client_longitude=0.000000&client_precision=0.000000&client_latitude=0.000000`)
    .then(function (data) {
