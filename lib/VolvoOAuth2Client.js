...
class VolvoOAuth2Client extends OAuth2Client {
     ...
     *
     * Forces `prompt=login` so the Volvo ID identity provider always presents
     * the credential screen, even when a previous session cookie is still
     * cached in the browser. This lets users sign in with a different account
     * during re-authorization.
     */
    onHandleAuthorizationURL({ scopes, state } = {}) {
        const baseUrl = super.onHandleAuthorizationURL({ scopes, state });
    ...
    async onHandleNotOK({ body, status }) {
        ...
        // Leave a breadcrumb (no PII, no VIN) so a later captured event carries
        // the trail of API failures that preceded it. Not raised as an issue -
        // API errors are far too frequent/transient to be useful individually.
        logger.addBreadcrumb({
            category: 'api',
            level: 'warning',
            message: `Volvo API error: ${errorMessage}`,
