    ...
    async onRequestHeaders({ headers }) {
        ...
        if (token.isCompletelyExpired && token.isCompletelyExpired()) {
            if (token.isRefreshable && token.isRefreshable()) {
                ...
                try {
                    ...
                    this.log('Token refreshed after expiry. New expiration:', token.getExpirationTime?.());

                    delete this._lastRefreshFailed;
                    delete this._lastRefreshError;
                } catch (error) {
                    this.error('Failed to refresh completely expired token:', error);
                    this._lastRefreshFailed = Date.now();
                    delete this._lastRefreshTime;

                    if (error.message && (error.message.includes('refresh token') || error.message.includes('invalid'))) {
                        this.log('Refresh token is invalid after expiry, emitting expired event');
                        this.emit('expired');
        ...
        if (token.isExpired && token.isExpired() && token.isRefreshable()) {
            ...
            if (shouldRefresh) {
                ...
                if (waitTime > 0) {
                    this.log(`Skipping proactive refresh (cooldown). Wait ${Math.round(waitTime/1000)}s (last error: ${this._lastRefreshError?.type || 'none'})`);
                }
            }
        }
